...
- Carbon `RegisterEventHotKey` for the global `Shift-Cmd-V` shortcut.
...
2. `AppDelegate` wires persistence, app state, the clipboard monitor, the hotkey,
   the status menu, and the SwiftUI panel.
...
6. `ClipboardPanelView` displays history, pinboards, search, retention, and
   paste actions.
