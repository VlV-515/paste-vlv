...
SourceForge mirror files for v1.3.1 are prepared locally for upload.
...
Cards use Paste-inspired dark surfaces, strong source-app headers, rounded corners, and macOS app icons. Text cards preserve copied line breaks, tabs, and indentation, so structured snippets remain readable. File cards keep a single-file thumbnail/path or a compact multi-file presentation; URL cards load a local rich preview when metadata is available. Drag across cards to select several items before deleting them. Inside a selected group, drag a card onto another card to reorder that group manually. Each card also has a contextual menu for paste, plain-text paste, favorite, pin, moving between groups, renaming, and deletion. Double-click its title to edit it inline; press `Return` to save.
